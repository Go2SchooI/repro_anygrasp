...
try:
    ...
    while True:
        ...
        if enable_rgb or enable_depth:
            frames = pipeline.wait_for_frames(4000 if (frame_count > 1) else 10000) # wait 10 seconds for first frame

        if enable_imu:
            imu_frames = imu_pipeline.wait_for_frames(4000 if (frame_count > 1) else 10000)
